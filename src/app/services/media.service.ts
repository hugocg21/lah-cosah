...
export class MediaService {
  ...
  getFolders(): Observable<string[]> {
    return this.storage.ref(this.basePath).listAll().pipe(map((result) =>
  ...
  getMediaByFolder(folder: string | null): Observable<Media[]> {
    ...
    return this.storage.ref(folderPath).listAll().pipe(mergeMap((result) => {
      const mediaObservables = result.items.filter((item) => item.name !== '.keep').map((item) =>from(item.getDownloadURL()).pipe(map((url) =>
        ({
          id: -1,
          name: item.name,
          url: url,
  ...
  getTotalMediaCount(): Observable<number> {
    return this.storage.ref(this.basePath).listAll().pipe(mergeMap((result) => {
      ...
      return forkJoin(folderObservables).pipe(map((folderCounts) => folderCounts.reduce((acc, count) => acc + count, rootMediaCount)));
    }));
  }
  ...
  private getMediaCountInFolder(folderPath: string): Observable<number> {
    return this.storage.ref(folderPath).listAll().pipe(mergeMap((result) => {
      ...
      return folderObservables.length > 0 ? forkJoin(folderObservables).pipe(map((counts) => counts.reduce((acc, count) => acc + count, mediaCount))) : of(mediaCount);
    }));
  }
  ...
  renameFolder(oldFolderName: string, newFolderName: string): Observable<void> {
    ...
    return this.storage.ref(oldFolderPath).listAll().pipe(mergeMap((result) => {
      const moveOps = result.items.map((item) => {
        const newItemRef = this.storage.ref(`${newFolderPath}/${item.name}`);
        return from(item.getDownloadURL()).pipe(mergeMap((url) => from(fetch(url).then((res) => res.blob()))), mergeMap((blob) => newItemRef.put(blob)), finalize(() => item.delete()));
      });

      const copyKeepFile = this.storage.ref(`${oldFolderPath}/.keep`).getDownloadURL().pipe(mergeMap((url) => from(fetch(url).then((res) => res.blob()))), mergeMap((blob) =>
        this.storage.ref(`${newFolderPath}/.keep`).put(blob)), catchError(() => of(null)));

      return forkJoin([...moveOps, copyKeepFile]).pipe(finalize(() => this.storage.ref(oldFolderPath).delete()));
    }), map(() => void 0));
  }
  ...
  uploadMedia(files: File[], folder: string | null): Observable<Media[]> {
    ...
    const uploads = files.map((file) => {
      ...
      return task.snapshotChanges().pipe(finalize(() => console.log(`Upload complete for: ${file.name}`)), last(), mergeMap(() => fileRef.getDownloadURL()), map((url) =>
        ({
          id: -1,
          name: file.name,
          url
  ...
  deleteFolder(folderName: string): Observable<void> {
    ...
    return folderRef.listAll().pipe(mergeMap((result) => {
  ...
  moveMediaToFolder(media: Media, folder: string | null): Observable<void> {
    ...
    return from(this.storage.refFromURL(media.url).getDownloadURL()).pipe(mergeMap((url) => from(fetch(url).then((res) => res.blob()))), mergeMap((blob) => newFileRef.put(blob)), finalize(() =>
      from(this.storage.refFromURL(media.url).delete())), map(() => void 0));
  }
}
