...
export class MediaService {
  ...
  getFolders(): Observable<string[]> {
    return this.storage.ref(this.basePath).listAll().pipe(map((result) => result.prefixes.map((prefix) => prefix.fullPath.split('/').pop()!)));
  }
  ...
  getMediaByFolder(folder: string | null): Observable<Media[]> {
    ...
    return this.storage.ref(folderPath).listAll().pipe(mergeMap((result) => {
      const mediaObservables = result.items.filter((item) => item.name !== '.keep').map((item) =>
        from(item.getDownloadURL()).pipe(map((url) => ({ id: -1, name: item.name, url: url, selected: false } as Media))));
  ...
  uploadMedia(files: File[], folder: string | null): Observable<Media[]> {
    ...
    const uploads = files.map((file) => {
      ...
      return task.snapshotChanges().pipe(finalize(() => fileRef.getDownloadURL()), mergeMap(() => fileRef.getDownloadURL()), map((url) => ({ id: -1, name: file.name, url })));
    });
  ...
  moveMediaToFolder(media: Media, folder: string | null): Observable<void> {
    ...
    return from(this.storage.refFromURL(media.url).getDownloadURL()).pipe(mergeMap((url) => from(fetch(url).then((res) => res.blob()))), mergeMap((blob) => newFileRef.put(blob)),
      finalize(() => from(this.storage.refFromURL(media.url).delete())), map(() => void 0));
  }
}
